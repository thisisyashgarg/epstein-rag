#!/usr/bin/env python3
"""
Epstein RAG Ingestion Script (text-only, no Vision API)
Uses PyMuPDF for proper font encoding support.
...
def make_chunk_id(filename: str, page: int, chunk_idx: int) -> str:
    raw = f"{filename}::p{page}::text::{chunk_idx}"
...
def process_pdf(pdf_path: Path, folder_name: str, collection) -> int:
    ...
    for page_num in range(len(doc)):
        ...
        for idx, chunk in enumerate(chunk_text(text)):
            cid = make_chunk_id(pdf_path.name, page_label, idx)
...
def main():
    print("🔍 Epstein RAG Ingestion (PyMuPDF, text-only)")
